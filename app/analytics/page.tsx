...
import { getTreeStage, getNextMilestone } from "@/constants/prayers";

export default function AnalyticsPage() {
  const { history, streaks, totalLifetimeCount, globalCommunityCount, badges } =
    useDhikrStore();
    ...
    <div className="px-4 sm:px-8 md:px-12 py-8 md:py-12 min-h-screen transition-all duration-300">
      ...
      >
        ...
        <div className="flex items-center gap-2 bg-gradient-to-r from-brand-500/10 to-brand-400/5 px-4 py-2 rounded-full border border-brand-500/20 shadow-sm">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-sm font-bold tabular-nums tracking-tight text-foreground leading-tight">
              {globalCommunityCount.toLocaleString()}
            </span>
          </div>
        </div>
      </motion.h1>
      ...
      <div className="grid grid-cols-2 gap-4 md:gap-8 mb-8 md:mb-12">
        ...
        >
          ...
          <span className="text-3xl md:text-5xl font-bold mb-1 md:mb-2 tabular-nums">
            {streaks.current}
          </span>
          <span className="text-xs md:text-sm text-foreground/50 tracking-wide uppercase font-medium">
            Day Streak
          </span>
        </motion.div>
          ...
          {last7Days.map((day, i) => {
            ...
            return (
              ...
              >
                ...
                <span className="text-[10px] md:text-xs text-foreground/50 font-medium uppercase">
                  {day.label}
                </span>
              </div>
            );
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 md:gap-4">
            {badges.map((b) => (
              ...
              >
                ...
                <span className="text-xs md:text-sm font-bold text-yellow-600 dark:text-yellow-500">
                  {b}
                </span>
              </div>
            ))}
