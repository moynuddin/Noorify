...
export default function Home() {
  const { currentDhikr, currentCount, increment, resetCounter, setDhikr, dailyChallenge } = useDhikrStore();
  ...
  const challengeDhikr = PREDEFINED_DHIKRS.find(d => d.id === dailyChallenge.dhikrId);
    ...
    <div className="flex flex-col items-center justify-center min-h-[90vh] px-4 sm:px-8 md:px-12 py-6 overflow-hidden">
      ...
      {!dailyChallenge.completed && challengeDhikr && (
        ...
        >
          ...
          <div>
            <h3 className="text-xs font-bold uppercase tracking-wider text-brand-600 dark:text-brand-400 mb-1">Daily Goal</h3>
            <p className="text-sm font-medium text-foreground">Complete 100x {challengeDhikr.name}</p>
          </div>
      ...
      <div className="w-full flex justify-between items-center mb-8 relative z-20">
        <div className="relative">
          ...
          >
            <span className="font-semibold text-sm tracking-wide">{currentDhikr.name}</span>
                ...
                {PREDEFINED_DHIKRS.map(d => (
                  ...
                  >
                    <div>
                      ...
                      {d.id === dailyChallenge.dhikrId && <span className="text-[9px] uppercase tracking-wider text-brand-500 font-bold block mt-0.5">Daily Goal</span>}
                    </div>
                    <span className="text-xs text-brand-500 bg-brand-500/10 px-2 py-1 rounded-md">{d.target}</span>
                  </button>
                ))}
              </motion.div>
            )}
        ...
        {currentDhikr.virtue && (
          ...
          >
            ...
            <p className="text-xs text-foreground/80 font-medium leading-relaxed italic text-left">
              "{currentDhikr.virtue}" <span className="text-brand-500 font-bold opacity-75">— {currentDhikr.virtueReference}</span>
            </p>
          </motion.div>
        )}
      </div>
      ...
      <div className="flex-1 flex items-center justify-center w-full max-w-[280px] sm:max-w-[340px] md:max-w-[420px] relative z-10 mb-8 mt-2 transition-all duration-300">
        <motion.button
          ...
          transition={breathingMode ? {
            duration: 8,
            repeat: Infinity,
            ease: "easeInOut"
          } : {}}
        >
          {/* Internal Glow on tap */}
          <motion.div 
            className="absolute inset-0 bg-brand-500/10 opacity-0 group-active:opacity-100 transition-opacity" 
          />
      ...
      <div className="mt-auto text-center relative z-10">
        <p className="text-[10px] sm:text-xs md:text-sm text-foreground/40 font-medium tracking-wide uppercase">Tap anywhere to count</p>
      </div>
    </div>
  );
}
