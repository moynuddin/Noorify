...
const SURAH_VIRTUES: Record<number, { text: string; reference: string }> = {
  1: {
    text: "The greatest Surah in the Quran... It is a cure for every disease.",
    reference: "Bukhari",
  },
  2: {
    text: "Recite Surah Al-Baqarah, for to take recourse to it is a blessing and to give it up is a cause of grief.",
    reference: "Muslim",
  },
  18: {
  ...
  36: {
    text: "Everything has a heart, and the heart of the Quran is Yaseen.",
    reference: "Tirmidhi",
  },
  67: {
    text: "There is a Surah of thirty verses which intercedes for a man until he is forgiven.",
    reference: "Abu Dawud",
  },
  112: {
    text: "By Him in Whose Hand is my life, it is equal to one-third of the Quran.",
    reference: "Bukhari",
  },
  113: {
    text: "The clearest words to seek refuge in Allah from evil.",
    reference: "An-Nasa'i",
  },
  114: {
    text: "The clearest words to seek refuge in Allah from evil.",
...
export default function SurahDetailClient({ id }: { id: number }) {
  ...
  const [translationData, setTranslationData] = useState<SurahData | null>(
    null,
  );
  const [transliterationData, setTransliterationData] =
    useState<SurahData | null>(null);
  const [loading, setLoading] = useState(true);
  const [errorOffline, setErrorOffline] = useState(false);

  const { markDownloaded, selectedTranslation, setTranslation } =
    useQuranStore();
  ...
  useEffect(() => {
    ...
    async function fetchSurah() {
      ...
      try {
        const [arJson, transJson, transliterationJson] = await Promise.all([
          fetchWithCache(
            `https://api.alquran.cloud/v1/surah/${id}/quran-uthmani`,
          ),
          fetchWithCache(
            `https://api.alquran.cloud/v1/surah/${id}/${selectedTranslation}`,
          ),
          fetchWithCache(
            `https://api.alquran.cloud/v1/surah/${id}/en.transliteration`,
          ),
        ]);
        ...
        if (
          arJson.code === 200 &&
          transJson.code === 200 &&
          transliterationJson.code === 200
        ) {
          setArabicData(arJson.data);
          setTranslationData(transJson.data);
          setTransliterationData(transliterationJson.data);
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          ...
          <h3 className="text-lg font-bold text-foreground mb-2">
            Surah Not Found Offline
          </h3>
          <p className="text-foreground/50 max-w-sm mx-auto text-sm">
            You are offline and haven&apos;t downloaded this Surah yet. Please
            connect to the internet to read or download it.
          </p>
        </div>
      ) : arabicData && translationData && transliterationData ? (
        <motion.div
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
        >
          {/* Header */}
          <div className="bg-linear-to-br from-brand-600 to-brand-500 text-white rounded-3xl p-6 md:p-10 mb-8 shadow-md relative overflow-hidden text-center">
            <div className="absolute top-[-20%] left-[-10%] w-64 h-64 bg-white/10 blur-3xl rounded-full pointer-events-none" />
            <h1 className="text-3xl md:text-5xl font-bold mb-2 relative z-10">
              {arabicData.englishName}
            </h1>
            <p className="text-brand-100 font-medium text-sm md:text-base relative z-10">
            ...
            <div className="flex items-center justify-center gap-2 mt-4 text-[10px] md:text-xs font-bold tracking-widest uppercase text-brand-100 relative z-10">
              <span className="bg-brand-400/30 px-2 py-0.5 rounded-sm">
          ...
          <div className="space-y-6 md:space-y-8">
            {arabicData.ayahs.map((ayah, i) => (
              ...
              >
                ...
                <p className="text-brand-700/80 dark:text-brand-200/80 text-sm md:text-base leading-relaxed italic font-medium mb-3">
                  {transliterationData.ayahs[i]?.text}
                </p>
